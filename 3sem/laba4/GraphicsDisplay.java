...
// Шаг 2 - Организовать цикл по всем точкам графика
        ...
        for (int i = 0; i < graphicsData.length; i++) {
            ...
            if (currentIt++ == selectedMarker) {
                ...
                Point2D.Double currentPoint = xyToPoint(graphicsData[i][0], graphicsData[i][1]);
                double currentPointChangedX = 0.0;
                double currentPointChangedY = 0.0;
                if (currentPoint.getX() <=  xyToPoint(0, 0).getX() ){
                    currentPointChangedX = currentPoint.getX() + 20;
                } else {
                    currentPointChangedX = currentPoint.getX() - 200;
                }

                if (currentPoint.getY() <= xyToPoint(0, 0).getY() ){
                    currentPointChangedY = currentPoint.getY() + 30;
                } else {
                    currentPointChangedY = currentPoint.getY() - 30;
                }
                ...
                if(graphicsData[i][1] >= graphicsData[i-1][1] &&
                    graphicsData[i][1] >= graphicsData[i+1][1] ||
                    graphicsData[i][1] <= graphicsData[i-1][1] &&
                    graphicsData[i][1] <= graphicsData[i+1][1]
                    ){
                    canvas.drawString(
                            extr + format.format(graphicsData[i][0]) +
                                    ", " +
                                    format.format(graphicsData[i][1]),
                            (int) currentPointChangedX,
                            (int) currentPointChangedY
                    );
                } else {
                    canvas.drawString(
                            format.format(graphicsData[i][0]) +
                                    ", " +
                                    format.format(graphicsData[i][1]),
                            (int) currentPointChangedX,
                            (int) currentPointChangedY
...
//                System.out.`println(Arrays.toString(point));
            ...
            verticalLine.setLine(
                    constructPoint(xyToPoint(graphicsData[i][0], graphicsData[i][1]), 0, 5),
                    constructPoint(xyToPoint(graphicsData[i][0], graphicsData[i][1]), -5, 0)
            );
    ...
    private boolean isAscending(String num) {
        ...
        for (int i = 0; i < num.length() && num.toCharArray()[i] != '.'; i++){
            System.out.println("Цифра в точке:");
            System.out.println(num.toCharArray()[i]);
            if(num.toCharArray()[i] == '-'){
                continue;
            }
            if(num.toCharArray()[i]%2!=0){
                return false;
            }
        }
